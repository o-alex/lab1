...
object task5 {
  def main(args: Array[String]) {
    ...
    val fSlicer = new VectorSlicer().setInputCol("allFeatures").setOutputCol("simple-features").setIndices(Array(0,1,2))
    val poli = new PolynomialExpansion().setInputCol("simple-features").setOutputCol("features").setDegree(2)
    val myLR = new LinearRegression().setMaxIter(10).setRegParam(0.3).setElasticNetParam(0.1);

    val lrStages = Array(regexTokenizer, arr2Vect, lSlicer, v2d, lShifter, fSlicer, poli, myLR)
